...
const CityMap: React.FC<CityMapProps> = ({
    ...
    useEffect(() => {
        ...
        // initMap();
        addLog('TEST MODE: Map Init SKIPPED. Should see RED screen.');
        ...
        return (
            <div className="fixed inset-0 w-full h-full bg-slate-100 z-0 overflow-hidden">
                ...
                <div
                    ...
                    style={{ background: 'red' }} // DEBUG
                >
                    <span className="text-4xl font-bold text-white bg-black/50 p-4 rounded">
                        CSS CHECK: IF YOU SEE THIS, LAYOUT IS OK
                    </span>
                </div>
                ...
                {/* Error Message */}
                {error && (
                    <div className="absolute inset-0 flex items-center justify-center bg-red-50 text-red-500 font-bold z-[1000]">
                        Map Load Error: {error}
                    </div>
                )}

                {/* Debug Logs Overlay */}
                <div className="absolute top-20 left-4 z-[9999] bg-black/70 text-white p-2 rounded text-xs font-mono pointer-events-none">
                    <div>Debug Info (v2):</div>
